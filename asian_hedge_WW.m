function asian_hedge_WW(CallPutFlag,k,b,m,SA,OptionPrice,r,sigma,minunit,zongsousu,TradingUnit,MarginPercent,CommissionPer,TraCostPer,RiskCoe,s,T,Tn,s1)
...
t1=T(1)-Tn(1);
n=length(Tn)+m;
delta(1,1)=EDiscreteAsianHHM('d', CallPutFlag,s(1),SA,k,t1,T(1) ,n,m ,r,b,sigma);
gamma=EDiscreteAsianHHM('g', CallPutFlag,s(1),SA,k,t1,T(1) ,n,m ,r,b,sigma);     
...
while (c<length(t))
    ...
    if t(c)==T(j)
       t1=T(j)-Tn(i);
       delta(j,1)=EDiscreteAsianHHM('d', CallPutFlag,s(j),SA,k,t1,T(j),n,m ,r,b,sigma);
    %     if isnan(delta(c,1));
    %         delta(c,1)=0;
    %     end
       gamma=EDiscreteAsianHHM('g', CallPutFlag,s(j),SA,k,t1,T(j),n,m ,r,b,sigma);     
...
for i=1:a
    commission(i,1)=-abs(changeP(i,1))*huazijia(i,1) * CommissionPer * TradingUnit*exp(-r*(T(1)-T(i)));
end
...
disp([num2str([s(1),s(end),SA,k,TotalPL,CumNodePL(end),CumCommission(end),CumInterest(end),OptionPrice])])
